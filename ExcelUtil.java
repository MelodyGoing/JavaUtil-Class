  ...
  private static <T> void fillSheet(
    ...
    for (int index = firstIndex; index <= lastIndex; index++) {
      ...
      for (int i = 0; i < enFields.length; i++) {
        ...
        //sheet.addCell(label);
        String filedValue = "";
        if ("number".equals(enFields[i])) {
          filedValue = (index + 1) + "";
        } else {
          filedValue = item.get(enFields[i]) == null ? null : item.get(enFields[i]).toString();
        }
        Label label = new Label(i, rowNo, filedValue);
        sheet.addCell(label);
      }
